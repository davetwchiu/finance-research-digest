...
<div class='card'><h2>Layman summary</h2>{plain_language_html}<p class='muted'>Bottom line first. The technical and verification stack stays collapsed below.</p></div>
...
<div class='card'><h2>At a glance</h2><p><span class='pill'>Status: {status}</span><span class='pill'>Score: {score.total}/100</span><span class='pill'>Setup: {score.ta}</span><span class='pill'>Fundamentals: {score.fundamentals}</span><span class='pill'>Freshness age: {_fmt(age_h,1,'h')}</span></p><p class='muted'>No evidence, no claim. Thin evidence stays labeled instead of being dressed up as conviction.</p></div>
{decision_html}
